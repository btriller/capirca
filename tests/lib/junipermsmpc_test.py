...
class JuniperMSMPCTest(parameterized.TestCase):
  ...
  @parameterized.named_parameters(
      dict(testcase_name='WithApplyGroups', apply_groups=True),
      dict(testcase_name='WithoutAapplyGroups', apply_groups=False),
  )
  def testApplicationSets(self, apply_groups):
    ...
    self.naming.GetServiceByProto.side_effect = [['67'], ['69']]
    msmpc = junipermsmpc.JuniperMSMPC(
        policy.ParsePolicy(header + RANGE_PORTS_TERM, self.naming),
        EXP_INFO)
  ...
  @parameterized.named_parameters(
      dict(testcase_name='true-apply-groups', option='true', want_logging=True, apply_groups=True),
      dict(testcase_name='True-apply-groups', option='True', want_logging=True, apply_groups=True),
      dict(testcase_name='syslog-apply-groups', option='syslog', want_logging=True, apply_groups=True),
      dict(testcase_name='local-apply-groups', option='local', want_logging=True, apply_groups=True),
      dict(testcase_name='disable-apply-groups', option='disable', want_logging=False, apply_groups=True),
      dict(testcase_name='log-both-apply-groups', option='log-both', want_logging=True, apply_groups=True),
      dict(testcase_name='true-no-apply-groups', option='true', want_logging=True, apply_groups=False),
      dict(testcase_name='True-no-apply-groups', option='True', want_logging=True, apply_groups=False),
      dict(testcase_name='syslog-no-apply-groups', option='syslog', want_logging=True, apply_groups=False),
      dict(testcase_name='local-no-apply-groups', option='local', want_logging=True, apply_groups=False),
      dict(testcase_name='disable-no-apply-groups', option='disable', want_logging=False, apply_groups=False),
      dict(testcase_name='log-both-no-apply-groups', option='log-both', want_logging=True, apply_groups=False),
  )
  ...
  @parameterized.named_parameters(
      dict(
          testcase_name='default-apply-groups', header=GOOD_HEADER,
          apply_groups=True,
      ...
      dict(
          ...
          direction='input'),
      dict(
          testcase_name='egress-apply-groups', header=GOOD_HEADER_EGRESS,
          apply_groups=True,
          direction='output'),
      dict(
          testcase_name='default-no-apply-groups', header=GOOD_HEADER,
          apply_groups=False,
          direction='input-output'),
      dict(
          ...
          direction='input'),
      dict(
          testcase_name='egress-no-apply-groups', header=GOOD_HEADER_EGRESS,
          apply_groups=False,
          direction='output'))
  ...
  @parameterized.named_parameters(
      ...
      dict(testcase_name='hopopt-apply-groups', protoname='hopopt', protonum='0', apply_groups=True),
      dict(testcase_name='vrrp-apply-groups', protoname='vrrp', protonum='112', apply_groups=True),
      dict(testcase_name='tcp-no-apply-groups', protoname='tcp', protonum='tcp', apply_groups=False),
      dict(testcase_name='hopopt-no-apply-groups', protoname='hopopt', protonum='0', apply_groups=False),
      dict(testcase_name='vrrp-no-apply-groups', protoname='vrrp', protonum='112', apply_groups=False),
  )
  def testProtocolAsNumber(self, protoname, protonum, apply_groups):
    ...
    msmpc = junipermsmpc.JuniperMSMPC(
        policy.ParsePolicy(
            header + GOOD_TERM_NUMERIC_PROTOCOL % protoname,
            self.naming), EXP_INFO)
    output = str(msmpc)
    if apply_groups:
